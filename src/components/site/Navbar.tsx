...
export function Navbar() {
  ...
  return (
    <>
      <header className={`fixed top-0 inset-x-0 z-40 transition-all duration-300 ${scrolled ? "backdrop-blur-md bg-background/80 shadow-sm" : "bg-transparent"} ${open ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 h-20 flex items-center justify-between">
          <a href="#top" className="flex items-center gap-2">
            <span className="h-6 w-6 rounded-t-full rounded-br-full rounded-bl-sm bg-foreground grid place-items-center"></span>
            <span className="font-bold text-lg tracking-tight text-foreground">GrowthTo</span>
          </a>
